...
class BraveWellKnownUriPlugin {
	...
	/**
	 * Delegates the request to the matchigitng (registered) class
	 *
	 * @param WP $wp
	 */
...
function well_known_uri( $query ) {
	...
	if ( is_array( $options ) ) {
		foreach ( $options as $key => $value ) {
			unset( $value );
			if ( strpos( $key, BRAVE_WELL_KNOWN_URI_MATCHER_SUFFIX ) !== 0 ) {
				continue;
			}
			...
			if ( ( empty( $suffix ) ) || ( strpos( $query[ BRAVE_WELL_KNOWN_URI_QUERY_VAR ], $suffix ) !== 0 ) ) {
				continue;
			}
...
class BraveWellKnownUriSettings {
	...
	public function page_init() {
		...
		if ( ! is_array( $options ) ) {
			...
			$newopts = array();
			for ( $i = 1, $j = 1;; $i++ ) {
				if ( ! isset( $options[ BRAVE_WELL_KNOWN_URI_MATCHER_CONTENTS . $i ] ) ) {
					break;
				}
				if ( empty( $options[ BRAVE_WELL_KNOWN_URI_MATCHER_CONTENTS . $i ] ) ) {
					continue;
				}
				...
				foreach ( $options as $key => $value ) {
					if ( stripos( strrev( $key ), $reversed_needle ) !== 0 ) {
						continue;
					}
			...
			for ( $j = 1;;
			$j++ ) {
				if ( ! isset( $newopts[ BRAVE_WELL_KNOWN_URI_MATCHER_CONTENTS . $j ] ) ) {
							break;
				}
			}
			$j = 1;
		...
		for ( $i = 1; $i <= $j; $i++ ) {
			add_settings_section(
				$section_prefix . $i,
				'Enter your Publisher Verification Code Below and click "Save Changes"',
				array( $this, 'print_section_info' ),
				$this->slug
			);
			add_settings_field(
				BRAVE_WELL_KNOWN_URI_MATCHER_CONTENTS . $i,
				$contents_title,
				array( $this, 'field_callback' ),
				$this->slug,
				$section_prefix . $i,
				array(
					'id'   => BRAVE_WELL_KNOWN_URI_MATCHER_CONTENTS . $i,
					'type' => 'textarea',
				)
			);
		}
	...
	public function sanitize_field( $input ) {
		...
		for ( $i = 1;; $i++ ) {
			if ( ! isset( $input[ BRAVE_WELL_KNOWN_URI_MATCHER_CONTENTS . $i ] ) ) {
				break;
			}
	...
	public function sanitize_type( $input, $id ) {
		...
		$token  = '/^([0-9A-Za-z' . "'" . preg_quote( '!#$%&*+^_`{|}~-', null ) . '])+$/';
		$word   = '/^([0-9A-Za-z' . preg_quote( '!#$%&*+^_`{|}~-', null ) . '])+$/';
		$string = '/^"([0-9A-Za-z' . preg_quote( '!#$%&*+^_`{|}~-', null ) . ']|(\\"))+"$/';
